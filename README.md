...
> **Status: in development.** Phases 0-2 are implemented (Tasks 1-16 of 25): the
...
> and confirms confirmed downlinks (command lifecycle `sent` → `acked` tracked
> from `event/txack` + `event/ack`). The dashboard and delivery hardening remain.
...
```
specs/        → design spec + implementation plan (Tasks 1-16 checked)
